<template>
    <AppLayout :title="`Email — ${domain.domain}`">
        ...
        <div v-if="!domain.mail_enabled" class="rounded-xl border border-amber-700 bg-amber-900/20 px-4 py-3 text-sm text-amber-300 mb-6">
        ...
        <div class="grid gap-6 xl:grid-cols-2">
            ...
            <div>
                ...
                <div v-if="domain.mail_enabled" class="mb-4 rounded-xl border border-gray-800 bg-gray-900 p-4">
                    <p class="text-xs font-medium text-gray-400 mb-3">New mailbox</p>
                    <form @submit.prevent="submitMailbox" class="space-y-3">
                        <div class="flex gap-2">
                            <input
                                v-model="mbForm.local_part"
                                type="text"
                                placeholder="user"
            ...
            <div>
                ...
                <!-- Add forwarder form -->
                <div v-if="domain.mail_enabled" class="mb-4 rounded-xl border border-gray-800 bg-gray-900 p-4">
                    <p class="text-xs font-medium text-gray-400 mb-3">New forwarder</p>
                    <form @submit.prevent="submitForwarder" class="space-y-3">
                        <input
                            v-model="fwdForm.source"
                            type="email"
                            :placeholder="`from@${domain.domain}`"
                            class="w-full rounded-lg border border-gray-700 bg-gray-800 px-3 py-2 text-sm text-gray-100 placeholder-gray-500 focus:border-indigo-500 focus:outline-none"
                        />
                        <p v-if="fwdForm.errors.source" class="text-xs text-red-400">{{ fwdForm.errors.source }}</p>
                        <input
                            v-model="fwdForm.destination"
                            type="email"
