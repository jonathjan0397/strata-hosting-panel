        ...
        <div class="max-w-2xl">
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="rounded-xl border border-gray-800 bg-gray-900 p-5">
                    <h3 class="text-sm font-semibold text-gray-200 mb-4">Client Details</h3>
                ...
                <div class="rounded-xl border border-gray-800 bg-gray-900 p-5">
                    ...
                    <p class="text-xs text-gray-500 mb-4">
                        Set to 0 for unlimited. Your remaining pool is shown below each field.
                    </p>
                    <div class="grid grid-cols-3 gap-4">
                        <FormField label="Disk (MB)" :error="form.errors.disk_limit_mb">
                            <input v-model.number="form.disk_limit_mb" type="number" min="0" class="field"
                                :max="remaining.disk_mb ?? undefined" placeholder="0" />
                            <p v-if="remaining.disk_mb !== null" class="mt-1 text-xs text-gray-500">
                                {{ remaining.disk_mb }} MB remaining
                            </p>
                        </FormField>
                        <FormField label="Bandwidth (MB)" :error="form.errors.bandwidth_limit_mb">
                            <input v-model.number="form.bandwidth_limit_mb" type="number" min="0" class="field"
                                :max="remaining.bandwidth_mb ?? undefined" placeholder="0" />
                            <p v-if="remaining.bandwidth_mb !== null" class="mt-1 text-xs text-gray-500">
                                {{ remaining.bandwidth_mb }} MB remaining
                            </p>
                        </FormField>
                        <FormField label="Max Domains" :error="form.errors.max_domains">
                            <input v-model.number="form.max_domains" type="number" min="0" class="field"
                                :max="remaining.domains ?? undefined" placeholder="0" />
                            <p v-if="remaining.domains !== null" class="mt-1 text-xs text-gray-500">
                                {{ remaining.domains }} remaining
                            </p>
                        </FormField>
                        <FormField label="Max Email Accounts" :error="form.errors.max_email_accounts">
                            <input v-model.number="form.max_email_accounts" type="number" min="0" class="field"
                                :max="remaining.email_accounts ?? undefined" placeholder="0" />
                            <p v-if="remaining.email_accounts !== null" class="mt-1 text-xs text-gray-500">
                                {{ remaining.email_accounts }} remaining
                            </p>
                        </FormField>
                        <FormField label="Max Databases" :error="form.errors.max_databases">
                            <input v-model.number="form.max_databases" type="number" min="0" class="field"
                                :max="remaining.databases ?? undefined" placeholder="0" />
                            <p v-if="remaining.databases !== null" class="mt-1 text-xs text-gray-500">
